...
(types, tokens, ratio) = sys.argv[4].split("/")
count_types= int(types) 
count_tokens= float(tokens)
count_ratio= float(ratio)

words_in_5KB= int(sys.argv[5])		#words in 5KB documnet
...
for file in files:
    ...
    if (len(distinct_words.keys()) >= count_types) and (word_count+0.0)/size >= count_tokens and (word_count+0.0)/tot_words >= count_ratio:
        os.system("cp %s %s" %(file, destDir))
         
    #list.append([len(distinct_words.keys()), (word_count+0.0)/size, (word_count+0.0)/tot_words, file])
    f.close()
